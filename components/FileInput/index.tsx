...
export default function FileInput({ name, ...props }: FileInputProps) {
  ...
  return (
    <div {...getRootProps()}>
      ...
      {!!files?.length ? (
        <S.DropzoneImageWrapper>
          <img src={URL.createObjectURL(files[0])} alt={files[0].name} />
        </S.DropzoneImageWrapper>
      ) : (
        <DropzoneView fileRejections={fileRejections} open={open} />
      )}
    </div>
  );
}
...
function DropzoneView({ fileRejections, open }: DropzoneViewProps) {
  if (fileRejections.length) {
    return (
      <S.DropzoneWrapper>
        <DropzoneIcon />
        <p>Esse arquivo não é aceito</p>
        <span>
